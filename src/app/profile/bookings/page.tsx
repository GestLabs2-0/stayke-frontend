...
function HostBookingsGated({ onEnable }: { onEnable: () => void }) {
  return (
    <div className="card-white flex flex-col items-center px-6 py-14 text-center">
      <span className="flex size-12 items-center justify-center rounded-full bg-accent-warm/15">
        <CalendarDays className="size-6 text-accent-warm" />
      </span>
      ...
      <p className="mt-2 max-w-sm font-sans text-sm text-[#434654]">
        Cambiá tu modo para gestionar tus reservas, ver el estado de cada una y
        controlar tus alojamientos.
      </p>
      <button
        type="button"
        onClick={onEnable}
...
export default function BookingsPage() {
  ...
  if (mode !== "host") {
    return <HostBookingsGated onEnable={() => setMode("host")} />;
  }
