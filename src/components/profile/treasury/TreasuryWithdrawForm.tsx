...
import { useWithdrawGuarantee } from "@/hooks/contracts/useWithdrawGuarantee";
import type {
  TreasuryFormValues,
  TreasuryWithdrawFormProps,
} from "@/types/profile/treasury";
import {
  createTreasuryWithdrawValidationSchema,
...
export function TreasuryWithdrawForm({
  ...
  const { run: withdraw, loading } = useWithdrawGuarantee();

  const validationSchema = useMemo(
    () => createTreasuryWithdrawValidationSchema(guaranteeBalance),
    [guaranteeBalance],
  );
  ...
  const handleSetPercentage = (pct: number) => {
    const calculated = (guaranteeBalance * pct).toFixed(2);
  ...
  return (
    <form onSubmit={formik.handleSubmit} className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="font-sans text-xs text-[#7C838F]">
            ...
            <span className="font-semibold text-[#171717]">
              ${guaranteeBalance.toFixed(2)} USDC
            </span>
          </span>
        </div>
      ...
      <div className="grid grid-cols-4 gap-2">
        {[
          ...
          <button
            ...
            disabled={loading || guaranteeBalance <= 0}
      ...
      <p className="font-sans text-xs leading-relaxed text-[#7C838F]">
        Solo puedes retirar tu garantía si no tienes reservas activas o
        compromisos pendientes en la plataforma.
      </p>

      <button
        type="submit"
        disabled={loading || guaranteeBalance <= 0}
        className="flex w-full min-h-[48px] cursor-pointer items-center justify-center gap-2 rounded-xl bg-[#171717] py-3.5 font-sans text-sm font-semibold text-white transition-all hover:bg-black active:scale-98 disabled:cursor-not-allowed disabled:opacity-60"
      >
        {loading ? (
          <>
